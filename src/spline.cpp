...
namespace squiggles {
    ...
    std::vector<ProfilePoint>
        ...
        for (auto vec = std::next(start); vec != end; ++vec) {
            ...
            std::vector<squiggles::SplineGenerator::GeneratedPoint> raw_path;
            try {
                raw_path = gen_raw_path(spline_start, spline_end, fast);
            }
            catch (...) {}
            // TODO: check if the vel or accel constraints are actually hit by the raw
            // path and return the raw path if not?
            std::vector<squiggles::ProfilePoint> profiled_path;
            try {
                profiled_path = parameterize(spline_start,
                    spline_end,
                    raw_path,
                    preferred_start_vel,
                    preferred_end_vel,
    ...
    std::vector<ProfilePoint>
        ...
        // trajectory states.
        std::vector <squiggles::ProfilePoint> time_adjusted;
        try {
            time_adjusted = integrate_constrained_states(constrainedStates);
        }
        catch (...) {}
